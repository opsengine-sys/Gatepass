...
function HeroBg() {
  return (
    <div className="absolute inset-0 pointer-events-none select-none overflow-hidden" aria-hidden>
      <div className="absolute" style={{ top: "-10%", left: "-5%", width: "70%", height: "80%", borderRadius: "50%", background: "radial-gradient(ellipse at center, rgba(192,107,44,0.22) 0%, transparent 70%)", filter: "blur(48px)" }} />
      <div className="absolute" style={{ top: "-15%", right: "-8%", width: "55%", height: "75%", borderRadius: "50%", background: "radial-gradient(ellipse at center, rgba(217,143,60,0.18) 0%, transparent 68%)", filter: "blur(56px)" }} />
      <div className="absolute" style={{ top: "10%", left: "25%", width: "50%", height: "60%", borderRadius: "50%", background: "radial-gradient(ellipse at center, rgba(251,191,114,0.15) 0%, transparent 65%)", filter: "blur(40px)" }} />
      <div className="absolute" style={{ bottom: "5%", left: "5%", width: "35%", height: "45%", borderRadius: "50%", background: "radial-gradient(ellipse at center, rgba(192,107,44,0.10) 0%, transparent 70%)", filter: "blur(36px)" }} />
      <DottedSurface className="absolute inset-0 w-full h-full" spacing={28} dotRadius={1.6} waveSpeed={0.9} mouseRadius={140} interactive={false} />
      <div className="absolute inset-x-0 bottom-0 h-32" style={{ background: "linear-gradient(to bottom, transparent, hsl(var(--background)))" }} />
    </div>
  );
}
...
function PageMotion() {
  return (
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden" aria-hidden>
      <DottedSurface className="absolute inset-0 w-full h-full opacity-[0.22] mix-blend-multiply" spacing={42} dotRadius={1.2} waveSpeed={0.55} mouseRadius={220} />
        ...
        <div className="max-w-6xl mx-auto px-6 h-[60px] flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1">
            {["Features", "How it Works", "Pricing"].map((item) => (
              <a key={item} href={`#${item.toLowerCase().replace(/ /g, "-")}`} className="text-[13px] font-medium text-muted-foreground hover:text-foreground px-3 py-1.5 rounded-lg hover:bg-secondary transition-all">{item}</a>
            ))}
          </nav>
      ...
      <main className="flex-1 relative z-10">
        <section className="relative pt-28 pb-24 px-6 overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto text-center relative z-10">
            ...
            <div className="mt-16 grid grid-cols-3 gap-4 max-w-lg mx-auto">{stats.map(s => <div key={s.label} className="bg-card border border-border rounded-2xl py-5 px-2 shadow-sm"><div className="text-[26px] font-bold text-primary leading-none">{s.num}</div><div className="text-[11.5px] text-muted-foreground mt-1.5">{s.label}</div></div>)}</div>
          </div>
        </section>
        <section id="features" className="py-20 px-6"><div className="max-w-5xl mx-auto"><div className="text-center mb-14"><div className="text-[11px] font-bold text-primary uppercase tracking-[0.12em] mb-3">Features</div><h2 className="text-[32px] md:text-[38px] font-bold tracking-tight text-foreground">Everything to secure your premises</h2><p className="text-[15px] text-muted-foreground mt-4 max-w-lg mx-auto leading-relaxed">From single-office startups to multi-location enterprises, GatePass scales with you.</p></div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{features.map(f => <div key={f.title} className="group bg-card border border-border rounded-2xl p-6 hover:border-primary/30 hover:shadow-md transition-all duration-200 cursor-default"><div className="flex items-start justify-between mb-4"><div className={cn("w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0", f.bg, f.color)}>{f.icon}</div><span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full", badgeBg[f.badge])}>{f.badge}</span></div><h3 className="font-semibold text-[14.5px] text-foreground mb-2 tracking-tight">{f.title}</h3><p className="text-[13px] text-muted-foreground leading-relaxed">{f.desc}</p></div>)}</div></div></section>
        <section id="how-it-works" className="py-20 px-6 bg-secondary/20"><div className="max-w-4xl mx-auto"><div className="text-center mb-14"><div className="text-[11px] font-bold text-primary uppercase tracking-[0.12em] mb-3">How it Works</div><h2 className="text-[32px] md:text-[38px] font-bold tracking-tight text-foreground">Up and running in minutes</h2></div><div className="grid grid-cols-1 md:grid-cols-3 gap-6">{steps.map((s, i) => <div key={s.num} className="relative bg-card border border-border rounded-2xl p-7 text-center hover:shadow-md transition-shadow">{i < steps.length - 1 && <div className="hidden md:block absolute top-1/2 -right-3 w-6 text-muted-foreground/30 text-xl font-thin z-10">›</div>}<div className="w-12 h-12 bg-primary/8 border border-primary/20 rounded-2xl flex items-center justify-center mx-auto mb-5"><span className="text-[17px] font-bold text-primary">{s.num}</span></div><h3 className="font-semibold text-[15px] text-foreground mb-2 tracking-tight">{s.title}</h3><p className="text-[13px] text-muted-foreground leading-relaxed">{s.desc}</p></div>)}</div></div></section>
        <section id="pricing" className="py-20 px-6"><div className="max-w-5xl mx-auto"><div className="text-center mb-14"><div className="text-[11px] font-bold text-primary uppercase tracking-[0.12em] mb-3">Pricing</div><h2 className="text-[32px] md:text-[38px] font-bold tracking-tight text-foreground">Simple, transparent pricing</h2><p className="text-[15px] text-muted-foreground mt-4">All plans include a 14-day free trial. No credit card required.</p></div><div className="grid grid-cols-1 md:grid-cols-3 gap-5 items-stretch">{plans.map(p => <div key={p.name} className={cn("relative bg-card border rounded-2xl p-7 flex flex-col transition-all hover:shadow-lg", p.highlight ? "border-primary shadow-[0_0_0_1px_hsl(var(--primary)/0.3),0_8px_32px_hsl(var(--primary)/0.12)]" : "border-border")}>{p.highlight && <div className="absolute -top-3.5 left-1/2 -translate-x-1/2"><span className="bg-primary text-white text-[11px] font-bold px-3.5 py-1 rounded-full shadow-sm">Most Popular</span></div>}<div className="mb-6"><div className="text-[19px] font-bold text-foreground tracking-tight">{p.name}</div><div className="text-[13px] text-muted-foreground mt-0.5">{p.tagline}</div><div className="mt-4 flex items-baseline gap-1"><span className="text-[36px] font-bold text-foreground leading-none tracking-tight">{p.price}</span>{p.period && <span className="text-[14px] text-muted-foreground">{p.period}</span>}</div></div><ul className="space-y-2.5 flex-1 mb-7">{p.features.map(f => <li key={f} className="flex items-start gap-2.5 text-[13px] text-foreground"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="w-4 h-4 text-primary flex-shrink-0 mt-0.5"><polyline points="20 6 9 17 4 12"/></svg>{f}</li>)}</ul><button onClick={() => setLocation(p.cta === "Contact Sales" ? "/sign-in" : "/sign-up")} className={cn("w-full py-2.5 rounded-xl text-[13.5px] font-semibold transition-all active:scale-[.98]", p.ctaVariant === "primary" ? "bg-primary text-white hover:bg-primary/90 shadow-sm" : "border border-border text-foreground hover:bg-secondary")}>{p.cta}</button></div>)}</div><p className="text-center text-[12px] text-muted-foreground mt-6">Contact us for volume discounts, annual billing, or custom enterprise contracts.</p></div></section>
        <section className="py-20 px-6 bg-secondary/20"><div className="max-w-3xl mx-auto"><div className="relative bg-primary rounded-3xl px-8 py-14 text-center text-white overflow-hidden"><div className="absolute inset-0 pointer-events-none" aria-hidden><div className="absolute -top-20 -right-20 w-64 h-64 rounded-full bg-white/5" /><div className="absolute -bottom-12 -left-12 w-48 h-48 rounded-full bg-black/10" /></div><div className="relative z-10"><h2 className="text-[26px] md:text-[32px] font-bold mb-4 leading-tight tracking-tight">Ready to modernise your<br/>visitor management?</h2><p className="text-[15px] text-white/75 mb-8 max-w-md mx-auto leading-relaxed">Join companies across India using GatePass to secure their offices and simplify gate operations.</p><div className="flex items-center justify-center gap-3 flex-wrap"><button onClick={() => setLocation("/sign-up")} className="bg-white text-primary font-semibold px-7 py-3 rounded-xl text-[14px] hover:bg-white/90 active:scale-[.98] transition-all shadow-sm">Start Free Trial</button><button onClick={() => setLocation("/sign-in")} className="border border-white/30 text-white font-medium px-7 py-3 rounded-xl text-[14px] hover:bg-white/10 active:scale-[.98] transition-all">Sign In</button></div></div></div></div></section>
      </main>
      <footer className="border-t border-border"><div className="max-w-6xl mx-auto px-6 py-10"><div className="flex flex-col md:flex-row items-start justify-between gap-8"><div className="max-w-[240px]"><div className="flex items-center gap-2 mb-3"><div className="w-7 h-7 bg-primary rounded-lg flex items-center justify-center"><svg viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" className="w-3.5 h-3.5"><rect x="3" y="4" width="14" height="10" rx="2"/><path d="M7 8h6M7 11h4"/><rect x="7" y="17" width="10" height="3" rx="1.5"/></svg></div><span className="font-bold text-[14px] tracking-tight text-foreground">GatePass</span></div><p className="text-[12.5px] text-muted-foreground leading-relaxed">Visitor & gate pass management for Indian corporate offices. Secure. Simple. Scalable.</p></div><div className="grid grid-cols-2 md:grid-cols-3 gap-8 text-[13px]"><div><div className="font-semibold text-foreground mb-3 text-[13px] tracking-tight">Product</div><ul className="space-y-2 text-muted-foreground">{["Features", "Pricing", "How it Works"].map(l => <li key={l}><a href={`#${l.toLowerCase().replace(/ /g, "-")}`} className="hover:text-foreground transition-colors">{l}</a></li>)}</ul></div><div><div className="font-semibold text-foreground mb-3 text-[13px] tracking-tight">Account</div><ul className="space-y-2 text-muted-foreground"><li><button onClick={() => setLocation("/sign-in")} className="hover:text-foreground transition-colors">Sign In</button></li><li><button onClick={() => setLocation("/sign-up")} className="hover:text-foreground transition-colors">Create Account</button></li></ul></div><div><div className="font-semibold text-foreground mb-3 text-[13px] tracking-tight">Contact</div><ul className="space-y-2 text-muted-foreground text-[12.5px]"><li>support@gatepass.in</li><li>+91 98765 00000</li></ul></div></div></div><div className="mt-8 pt-6 border-t border-border flex flex-col md:flex-row items-center justify-between gap-3 text-[12px] text-muted-foreground"><span>© {new Date().getFullYear()} GatePass Technologies Pvt. Ltd.</span><div className="flex items-center gap-4"><a href="#" className="hover:text-foreground transition-colors">Privacy Policy</a><a href="#" className="hover:text-foreground transition-colors">Terms of Service</a></div></div></div></footer>
    </div>
  );
}
