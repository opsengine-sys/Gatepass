...
const featureHighlights = [
  { title: "Visitor checks", desc: "Faster check-ins with photo and badge flows." },
  { title: "Brand customization", desc: "Control colors, logos, fonts, and badge templates." },
  { title: "Multi-office control", desc: "Run offices, users, and gate passes from one place." },
];
      ...
      <main className="flex-1 relative z-10">
        <section className="relative pt-28 pb-24 px-6 overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto text-center relative z-10">
            ...
            <div className="mt-16 grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-3xl mx-auto">{featureHighlights.map((s, i) => <div key={s.title} className={cn("bg-card border border-border rounded-2xl py-5 px-4 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg animate-[fade-in_0.6s_ease-out_both]", i === 1 ? "delay-75" : i === 2 ? "delay-150" : "")}><div className="text-[14px] font-semibold text-foreground leading-tight">{s.title}</div><div className="text-[11.5px] text-muted-foreground mt-1.5">{s.desc}</div></div>)}</div>
          </div>
        </section>
        ...
        <section id="how-it-works" className="py-20 px-6 bg-secondary/20">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">{steps.map((s, i) => <div key={s.num} className="relative bg-card border border-border rounded-2xl p-7 text-center transition-all duration-300 hover:-translate-y-1 hover:shadow-lg animate-[fade-in_0.6s_ease-out_both]">{i < steps.length - 1 && <div className="hidden md:block absolute top-1/2 -right-3 w-6 text-muted-foreground/30 text-xl font-thin z-10">›</div>}<div className="w-12 h-12 bg-primary/8 border border-primary/20 rounded-2xl flex items-center justify-center mx-auto mb-5 transition-transform duration-300 group-hover:scale-105"><span className="text-[17px] font-bold text-primary">{s.num}</span></div><h3 className="font-semibold text-[15px] text-foreground mb-2 tracking-tight">{s.title}</h3><p className="text-[13px] text-muted-foreground leading-relaxed">{s.desc}</p></div>)}</div>
          </div>
        </section>
