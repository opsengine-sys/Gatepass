...
export function BadgeModal({ visitor: v, officeFull, onClose }: Props) {
  if (!v) return null;
  const color = TYPE_COLORS[v.type] ?? "#c06b2c";
  ...
  const handlePrint = () => {
    ...
    <div class="card">
      ...
      <div class="inner">
        ...
        <div class="photo">${v.photoUrl ? `<img src="${v.photoUrl}"/>` : `<div class="init">${initials(v.name)}</div>`}</div>
        <div class="name">${v.name}</div>
        <div class="company">${v.company || ""}</div>
        <div class="pill">${v.type}</div>
        <div class="vid">
        ...
        <div class="grid">
          <div class="cell"><div class="key">Host</div><div class="val">${v.hostName || "—"}</div></div>
          <div class="cell"><div class="key">Office</div><div class="val">${officeShort}</div></div>
          <div class="cell"><div class="key">Date</div><div class="val">${fmtDate(v.checkInTime ?? v.createdAt)}</div></div>
          <div class="cell"><div class="key">Time In</div><div class="val">${fmtTime(v.checkInTime ?? v.createdAt)}</div></div>
        </div>
      </div>
      ...
      <DialogContent className="max-w-[720px] max-h-[93vh] overflow-y-auto">
        ...
        <div className="flex gap-5 flex-wrap py-2 items-start">
          <div className="flex-shrink-0">
            <BadgeCard visitor={v} officeFull={officeFull} />
          </div>
...
function BadgeCard({ visitor: v, officeFull }: { visitor: Visitor; officeFull: string }) {
  const color = TYPE_COLORS[v.type] ?? "#c06b2c";
  const officeShort = officeFull.split("—")[0]?.trim() ?? officeFull;
      ...
      <div className="px-5 py-5">
        ...
        <div className="grid grid-cols-2 gap-1.5 text-left mb-2">
          {[
            ["Host", v.hostName ?? "—"],
            ["Office", officeShort],
            ["Date", fmtDate(v.checkInTime ?? v.createdAt)],
            ["Time In", fmtTime(v.checkInTime ?? v.createdAt)],
          ].map(([k, val]) => (
